...
function pagamento() {
	...
	$.ajax({
		...
		url: "_assets/scripts/pagamento.php",
...
$("#formPagamento").on("submit", function (event) {
	...
	var paymentMethod = $('#paymentMethod').val(); //document.querySelector('input[name="paymentMethod"]:checked').value;
	...
	if (paymentMethod == 'creditCard') {
		...
		PagSeguroDirectPayment.createCardToken({
			...
			expirationMonth: decodeURI($('#mesValidade').val()), // Mês da expiração do cartão

			expirationYear: decodeURI($('#anoValidade').val()), // Ano da expiração do cartão, é necessário os 4 dígitos.
...
function recupHashCartao() {
	...
	PagSeguroDirectPayment.onSenderHashReady(function (retorno) {
		...
		if (retorno.status == 'error') {
			...
			var dados = $("#formPagamento").serializeArray();
			...
			$.ajax({
				...
				url: "_assets/scripts/proc_pag.php",
				...
				success: function (retorna) {
					console.log("Sucesso " + JSON.stringify(retorna));
					 ...
					 window.open(retorna['dados']['paymentLink']);
...
$(document).ready(function(){
	...
	$(window).on('load', function() {
		$('#paymentMethod').val($('input[name=formaPagamento]:checked').val());
		if($('input[name=formaPagamento]:checked').val() == "boleto"){
			$(".cart").hide();
			$(".cart > input").removeAttr('required');
		}else{
			$(".cart").show();
			$(".cart > input").prop('required', 'true');
		}
	});

	// ao clicar para alterar
	$('.formpag1').on('change', function() {
		$('#paymentMethod').val($('input[name=formaPagamento]:checked').val());
		if($('input[name=formaPagamento]:checked').val() == "boleto"){
			$(".cart").hide();
			$(".cart > input").removeAttr('required');
